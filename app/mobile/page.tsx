...
export default function Home() {
  ...
  const checkBaseEnergy = async () => {
    ...
    try {
      ...
      if (active) {
        ...
        setBaseEnergyStatus(`ENERGY ACTIVE • ${formatEnergyTimeLeft(Number(left))}`);
      } else {
  ...
  const applySocketState = (state: any) => {
    ...
    if (
      ...
      if (lastCountdownKeyRef.current !== countdownKey) {
        ...
        const serverNow =
          typeof serverNowRaw === "number" ? serverNowRaw : Number(serverNowRaw);
        const startAtMs =
  ...
  useEffect(() => {
    checkBaseEnergy();
    const timer = setInterval(() => { checkBaseEnergy(); }, 60_000);
  ...
  useEffect(() => {
    ...
    socket.on("arena-vote-start", ({ votes }) => {
      ...
      setVotedArena(null);
      setArenaVotes({ host: votes?.host ?? null, guest: votes?.guest ?? null });
    });
  ...
  useEffect(() => {
    ...
    const down = (e: PointerEvent) => {
      // FIX: preventDefault stops scroll stealing and fixes Base App touch
      e.preventDefault();
    ...
    const move = (e: PointerEvent) => {
      ...
      const playerLines = linesRef.current.filter((line) => line.owner === "player");

      if (playerLines.length >= 2) {
        const firstPlayerLineIndex = linesRef.current.findIndex((line) => line.owner === "player");
    ...
    // FIX: passive: false is required for preventDefault to work in Base App WebView
    ...
    const loop = () => {
      ...
      if (activeArena === "base") {
        ...
        const stadiumGlow = ctx.createRadialGradient(W / 2, H / 2, 20, W / 2, H / 2, H / 1.05);
        ...
        for (let x = 38; x <= W - 38; x += 24) {
          ...
          ctx.beginPath(); ctx.arc(x, 15, 2.2, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(255,255,255,${blink})`;
          ctx.shadowColor = "#0052FF"; ctx.shadowBlur = 10; ctx.fill();
          ctx.beginPath(); ctx.arc(x, H - 15, 2.2, 0, Math.PI * 2); ctx.fill();
        }
        ctx.strokeStyle = `rgba(239,68,68,${0.35 + Math.sin(frame * 0.06) * 0.15})`;
        ctx.lineWidth = 2; ctx.shadowColor = "#ef4444"; ctx.shadowBlur = 18;
        ctx.beginPath();
        ctx.moveTo(30, 30); ctx.lineTo(30, H - 30);
        ctx.moveTo(W - 30, 30); ctx.lineTo(W - 30, H - 30);
        ctx.stroke();
        ctx.shadowBlur = 0;
        ctx.restore();
        ...
        ctx.lineWidth = 1;
        for (let x = 36; x < W; x += 52) { ctx.beginPath(); ctx.moveTo(x, 18); ctx.lineTo(x, H - 18); ctx.stroke(); }
        for (let y = 56; y < H; y += 70) { ctx.beginPath(); ctx.moveTo(18, y); ctx.lineTo(W - 18, y); ctx.stroke(); }

        ctx.save();
        const ringPulse = 0.55 + Math.sin(frame * 0.045) * 0.2;
        ctx.strokeStyle = `rgba(34,211,238,${ringPulse})`;
        ctx.lineWidth = 3; ctx.shadowColor = "#22d3ee"; ctx.shadowBlur = 18;
        ...
        ctx.lineWidth = 1.5;
        ctx.beginPath(); ctx.ellipse(0, 0, 112, 40, 0, 0, Math.PI * 2); ctx.stroke();
        ctx.beginPath(); ctx.ellipse(0, 0, 78, 26, Math.PI / 2.8, 0, Math.PI * 2); ctx.stroke();
        ctx.restore();
        ...
        ctx.lineWidth = 1;
        for (let x = 40; x < W; x += 40) { ctx.beginPath(); ctx.moveTo(x, 18); ctx.lineTo(x, H - 18); ctx.stroke(); }
        for (let y = 60; y < H; y += 60) { ctx.beginPath(); ctx.moveTo(18, y); ctx.lineTo(W - 18, y); ctx.stroke(); }

        for (let y = 70; y < H - 60; y += 88) {
          ctx.fillStyle = "rgba(251,191,36,0.12)";
          ctx.fillRect(18, y, 16, 50); ctx.fillRect(W - 34, y, 16, 50);
          ctx.fillStyle = "rgba(251,191,36,0.22)";
          ctx.fillRect(14, y - 5, 24, 6); ctx.fillRect(W - 38, y - 5, 24, 6);
        }

        ctx.save();
        const templePulse = 0.5 + Math.sin(frame * 0.045) * 0.18;
        ctx.strokeStyle = `rgba(251,191,36,${templePulse})`;
        ctx.lineWidth = 3; ctx.shadowColor = "#fbbf24"; ctx.shadowBlur = 18;
        ctx.strokeRect(12, 12, W - 24, H - 24);
      ...
      if (score.player === 6 && score.ai === 6) {
        ...
        ctx.shadowBlur = 26;
        ctx.fillText(
          activeArena === "base" ? "FINAL BLOCK" : activeArena === "space" ? "ORBIT CLASH" : activeArena === "temple" ? "FINAL RUNE" : "FINAL CLASH",
          W / 2, H / 2 - 95
        );
        ctx.shadowBlur = 0;
        ...
        ctx.shadowBlur = 24;
        ctx.fillText(
          activeArena === "base" ? "BASE POINT" : activeArena === "space" ? "ORBIT POINT" : activeArena === "temple" ? "CHAIN POINT" : "MATCH POINT",
          W / 2, H / 2 - 95
        );
        ctx.shadowBlur = 0;
      }
      ...
      if (roundActive) {
        if (gameModeRef.current === "online") {
          ...
          if (distance < 0.9 || distance > 90) {
            ball.x = predictedX; ball.y = predictedY;
          } else {
            ball.x += dx * 0.32; ball.y += dy * 0.32;
          }
          ...
          for (let s = 0; s < steps; s++) {
            ...
            for (const line of linesRef.current) {
              ...
              if (lenSq === 0) continue;
              const t = Math.max(0, Math.min(1, ((ball.x - line.x1) * lineDx + (ball.y - line.y1) * lineDy) / lenSq));
              ...
              if (dist < ball.r + 6) {
                ...
                if (overlap > 0) { ball.x += nx * (overlap + 0.75); ball.y += ny * (overlap + 0.75); }
                hitLine = line;
                line.life = 0;
                break;
              }
      ...
      if (roundActive && gameModeRef.current !== "online" && !goalLockRef.current && ball.y < 22) {
        goalLockRef.current = true;
        score.player++;
        if (score.player >= 7) {
          pauseRef.current = true;
          ...
          setTimeout(() => { pauseRef.current = false; startCountdown(roundStartAt); }, 0);
        }
        score.messageLife = 70;
      }

      if (roundActive && gameModeRef.current !== "online" && !goalLockRef.current && ball.y > H - 22) {
        goalLockRef.current = true;
        score.ai++;
        if (score.ai >= 7) {
          pauseRef.current = true;
      ...
      linesRef.current = linesRef.current
        .map((line) => ({ ...line, life: line.life - 1 }))
      ...
      for (let i = 0; i < trailRef.current.length; i++) {
        ...
        ctx.fillStyle =
          activeArena === "base" ? `rgba(239,68,68,${alpha * 0.30})`
          : activeArena === "space" ? `rgba(34,211,238,${alpha * 0.30})`
          : activeArena === "temple" ? `rgba(251,191,36,${alpha * 0.30})`
          : `rgba(0,82,255,${alpha * 0.28})`;
        ctx.shadowColor =
          activeArena === "base" ? "#ef4444"
          : activeArena === "space" ? "#22d3ee"
          : activeArena === "temple" ? "#fbbf24"
          : "#0052FF";
  ...
  const mobileFindMatch = () => {
    ...
    setRoomCode(null);
    setOnlineStatus("SEARCHING OPPONENT...");
    socketRef.current?.emit("find-match", { address, username: readyUsername });
  };
  ...
  return (
    <main
      className={`w-screen bg-black text-white select-none ${screenShake ? "goal-shake" : ""}`}
      style={{
        // FIX: Always fixed + overflow-y scroll for menu, hidden for game.
        // This prevents the page scroll from eating touch events in Base App.
        position: "fixed",
        inset: 0,
        height: "100dvh",
        overflowY: screen === "menu" ? "scroll" : "hidden",
        overflowX: "hidden",
        // FIX: pan-y on menu allows scroll without triggering pull-to-refresh in Base App.
        touchAction: screen === "menu" ? "pan-y" : "none",
    ...
    >
      ...
      {!showSplash && screen === "menu" && (
        <section
          className="relative z-10 min-h-[100dvh] w-full flex flex-col px-4 pt-4 pb-28"
          style={{ touchAction: "pan-y" }}
        >
          <img src="/splash.png" alt="" className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-20 blur-sm" />
          <div className="pointer-events-none absolute inset-0 bg-black/80" />

          {/* Header */}
          <div className="relative z-10 flex items-center justify-between">
            <div>
              <p className="text-[10px] font-black tracking-[0.35em] text-[#0052FF]">MOBILE MODE</p>
              <h1 className="mt-1 text-3xl font-black tracking-[-0.04em]">BOING BATTLE</h1>
            </div>
            <button
              type="button"
              {...mobileTap(() => { openConnectModal?.(); })}
              className="h-11 px-4 rounded-2xl border border-white/15 bg-white/5 text-[10px] font-black tracking-[0.18em] active:scale-95"
            >
              {isConnected && address ? `${address.slice(0, 4)}...${address.slice(-3)}` : "WALLET"}
            </button>
          </div>
          ...
          <div className="relative z-10 mt-4 rounded-[28px] border border-white/10 bg-black/45 p-3 shadow-[0_0_35px_rgba(0,82,255,0.20)]">
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                {...mobileTap(() => { setSocketRegion("EU"); navigator.vibrate?.(15); })}
                className={`flex h-11 items-center justify-center rounded-2xl text-xs font-black tracking-[0.18em] active:scale-95 ${
                  socketRegion === "EU" ? "bg-[#0052FF] text-white" : "bg-white/5 text-white/50 border border-white/10"
                }`}
              >
                EU
              </button>
              <button
                type="button"
                {...mobileTap(() => { setSocketRegion("US"); navigator.vibrate?.(15); })}
                className={`flex h-11 items-center justify-center rounded-2xl text-xs font-black tracking-[0.18em] active:scale-95 ${
                  socketRegion === "US" ? "bg-[#0052FF] text-white" : "bg-white/5 text-white/50 border border-white/10"
                }`}
              >
                US
              </button>
            ...
            {ARENA_OPTIONS.map((item) => (
              <button
                ...
                className={`h-[74px] rounded-[22px] border p-3 text-left active:scale-95 bg-gradient-to-br ${item.previewClass} ${
                  arena === item.key ? item.selectedClass : "border-white/10 text-white/45"
                }`}
              >
                <div className={`h-2 w-2 rounded-full ${item.dot}`} />
                <div className="mt-2 text-xs font-black tracking-[0.15em]">{item.title}</div>
                <div className="mt-1 text-[9px] font-black tracking-[0.15em] opacity-60">{item.subtitle}</div>
              </button>
            ))}
          </div>

          {/* Action Buttons */}
          <div className="relative z-10 mt-auto space-y-3 pb-2 pt-6">
            {!baseEnergyActive && (
              <button
                type="button"
            ...
            <button
              ...
              className="flex h-16 w-full items-center justify-center rounded-[28px] border border-white/15 bg-white/5 text-white text-base font-black tracking-[0.22em] active:scale-95 disabled:opacity-70"
            >
        ...
        <section className="relative h-[100dvh] w-screen flex items-center justify-center bg-black overflow-hidden">
          ...
          >
            ...
            <div className="pointer-events-none absolute right-3 top-3 z-30 rounded-2xl border border-white/10 bg-black/45 px-3 py-2 text-right backdrop-blur">
              <div className="text-[9px] font-black tracking-[0.18em] text-white/35">{getArenaLabel(arena)}</div>
      ...
      {showDifficulty && (
        <div className="absolute inset-0 z-[200] flex items-end justify-center bg-black/78 p-4 backdrop-blur">
          <div className="w-full max-w-[390px] rounded-[32px] border border-white/10 bg-[#050508] p-4 shadow-[0_0_45px_rgba(0,82,255,0.28)]">
            <p className="text-center text-[10px] font-black tracking-[0.35em] text-[#0052FF]">SELECT DIFFICULTY</p>
      ...
      {matchFound && !showArenaVote && (
        <div className="absolute inset-0 z-[220] flex items-center justify-center bg-black/86 p-4 backdrop-blur">
          <div className="w-full max-w-[390px] rounded-[32px] border border-[#0052FF]/35 bg-[#030309] p-6 text-center shadow-[0_0_55px_rgba(0,82,255,0.35)]">
            <p className="text-[10px] font-black tracking-[0.35em] text-[#0052FF]">MATCH FOUND</p>
            <div className="mt-5 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
                <div className="truncate text-sm font-black tracking-[0.12em]">{playerDisplayName}</div>
              </div>
              <div className="text-xl font-black text-[#0052FF]">VS</div>
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
                <div className="truncate text-sm font-black tracking-[0.12em]">{rivalDisplayName}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {showArenaVote && (
        <div className="absolute inset-0 z-[230] flex items-end justify-center bg-black/84 p-4 backdrop-blur">
          <div className="w-full max-w-[420px] rounded-[32px] border border-white/10 bg-[#050508] p-4">
            <p className="text-center text-[10px] font-black tracking-[0.35em] text-[#0052FF]">VOTE ARENA</p>
                ...
                return (
                  <button
                    ...
                    {...mobileTap(() => {
                      ...
                      socketRef.current?.emit("vote-arena", { roomCode: roomIdRef.current, arena: item.key });
                    })}
                    className={`min-h-[92px] rounded-[22px] border p-3 text-left bg-gradient-to-br ${item.previewClass} active:scale-95 ${
                      votedArena === item.key || myVote === item.key ? item.selectedClass : "border-white/10 text-white/45"
                    }`}
                  >
                    <div className={`h-2 w-2 rounded-full ${item.dot}`} />
                    <div className="mt-2 text-xs font-black tracking-[0.15em]">{item.title}</div>
      ...
      {winner && (
        <div className="absolute inset-0 z-[260] flex items-center justify-center bg-black/88 p-4 backdrop-blur">
          <div className="w-full max-w-[390px] rounded-[34px] border border-[#0052FF]/35 bg-[#030309] p-6 text-center shadow-[0_0_60px_rgba(0,82,255,0.35)]">
            <p className="text-[10px] font-black tracking-[0.35em] text-[#0052FF]">GAME OVER</p>
            <h2 className="mt-3 text-4xl font-black tracking-[-0.06em]">{winner}</h2>
